...
interface UserBalancesBoxProps {
  passZC: number; // in smallest units
  passSol: number; // in smallest units (quote token)
  failZC: number; // in smallest units
  failSol: number; // in smallest units (quote token)
  zcPrice: number | null;
  solPrice: number | null; // quote token price
  baseDecimals: number; // Required - token decimals for base token
  quoteDecimals: number; // Required - token decimals for quote token
...
export function UserBalancesBox({ passZC, passSol, failZC, failSol, zcPrice, solPrice, baseDecimals, quoteDecimals, quoteDisplayDecimals = 4, tokenSymbol, quoteSymbol, quoteIcon }: UserBalancesBoxProps) {
      ...
      <div className="text-theme-text flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <div className="flex items-center gap-1 group relative cursor-default" title={solPrice ? formatCurrency((passSol / quoteMultiplier) * solPrice, 2) : 'Price unavailable'}>
              <span>{formatNumber(passSol / quoteMultiplier, quoteDisplayDecimals)}</span>
              <QuoteIcon />
              {solPrice && (
                <span className="absolute right-0 top-full mt-1 px-2 py-1 bg-theme-input border border-theme-border-divider rounded text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                  {formatCurrency((passSol / quoteMultiplier) * solPrice, 2)}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-theme-text-tertiary">|</span>
            <div className="flex items-center gap-1 group relative cursor-default" title={solPrice ? formatCurrency((failSol / quoteMultiplier) * solPrice, 2) : 'Price unavailable'}>
              <span>{formatNumber(failSol / quoteMultiplier, quoteDisplayDecimals)}</span>
              <QuoteIcon />
              {solPrice && (
                <span className="absolute right-0 top-full mt-1 px-2 py-1 bg-theme-input border border-theme-border-divider rounded text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                  {formatCurrency((failSol / quoteMultiplier) * solPrice, 2)}
                </span>
              )}
            </div>
          </div>
